...
const printCorrectValue = (value: string | number) =>
  Number.isNaN(Number(value)) ? `'${value}'` : `${value} | '${value}'`;
const getTypeUnion = (json: Record<string, unknown>) =>
  Object.keys(json).reduce((acc, key, index) => acc.concat(`${index !== 0 ? '| ' : ''}${printCorrectValue(key)} `), '');

const reduceTokensJson = (tokens: Record<string, unknown>): string =>
...
const run = () => {
  ...
  colors: ${Object.keys(tokens.color as Record<string, unknown>)
    .filter((i) => !colorsToExclude.includes(i))
    .map((item) => `'${item}'`)
    .join('|')};
};`;
